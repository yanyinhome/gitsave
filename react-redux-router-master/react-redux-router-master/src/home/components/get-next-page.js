...
!(function(GetNextPage) {
    ...
    GetNextPage.prototype.GetNextPageData = function(el) {
        ...
        for (var attr in this.data) {
            aData.push(attr + '=' + filter(this.data[attr]));
        }
        sData = aData.join('&');

        this.xhr = new XMLHttpRequest();
        try {
            if (this.type == 'GET') { //get方式请求
                sData = this.url + '?' + sData;
                this.xhr.open(this.type, sData + '&' + new Date().getTime(), this.async);
                this.xhr.send();
            } else { //post方式请求
